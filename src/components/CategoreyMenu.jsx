    ...
    <div className='ml-6'>
        ...
        <button onClick={()=>dispatch(setCategory('All'))} className={`px-3 py-2 bg-gray-200 font-bold rounded-lg  hover:bg-green-500 hover:text-white ${ SelectedCategory==="All"  && "bg-green-500 text-white" }`}>
          ...
          {catogries.map((category,index)=>(
            <button key={index} onClick={()=>dispatch(setCategory(category))} className={`px-3 py-2 bg-gray-200 font-bold rounded-lg  hover:bg-green-500 hover:text-white ${ SelectedCategory==="All"  && "bg-green-500 text-white" }`}>
              {category}
              </button>
          ))}
